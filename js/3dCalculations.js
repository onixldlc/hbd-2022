	...
	cardsGLobal.forEach(cards => {
		['mousemove','touchmove'].forEach( e => 
			cards.addEventListener(e, function(e) {
				...
				this.style = tf;
				nameReflection.style = `transform: perspective(1000px) translateY(17px) rotateX(${(ty*2)+50-170}deg) rotateY(${tx/8}deg)`
				descReflection.style = `transform: perspective(1000px) translateY(17px) rotateX(${(ty*2)+50-170}deg) rotateY(${tx/8}deg)`
				if ( e.type === "touchmove" ) {
					return false; 
				}
